import { jsPDF } from 'jspdf';
import {transliterate} from "./transliterate";

export const generatePdf = async (parsedData, options) => {
  try {
    const doc = new jsPDF();
    let yPosition = 20;
    const margin = 20;
    const lineHeight = options.fontSize * options.lineHeight; // Высота строки с учетом интервала
    const fixedTextOffset = 80;

    // Используем моноширинный шрифт
    doc.setFont('courier');
    doc.setFontSize(options.fontSize);

    // Заголовок
    if (options.showTitle) {
      const titleSize = Math.max(options.fontSize + 4, 14);
      doc.setFontSize(titleSize);
      doc.setFont(undefined, 'bold');
      const title = parsedData.title ? transliterate(parsedData.title) : 'Song';
      doc.text(title, 105, yPosition, { align: 'center' });
      yPosition += titleSize * options.lineHeight + 2;
      doc.setFontSize(options.fontSize);
    }

    // Метаданные
    if (options.showTempo) {
      doc.setFont(undefined, 'normal');
      const metaInfo = [
        parsedData.tempo && `T: ${parsedData.tempo}`,
        parsedData.meter && `M: ${parsedData.meter}`,
        parsedData.key && `K: ${parsedData.key}`,
        parsedData.composer && `By: ${transliterate(parsedData.composer)}`
      ].filter(Boolean).join(' | ');

      doc.text(metaInfo, 105, yPosition, { align: 'center' });
      yPosition += lineHeight;
    }

    yPosition += 4;

    // Секции
    if (parsedData.sections && parsedData.sections.length > 0) {
      for (const section of parsedData.sections) {
        // Заголовок секции
        const sectionSize = options.fontSize + 1;
        doc.setFontSize(sectionSize);
        doc.setFont(undefined, 'bold');
        const sectionName = section.name ? transliterate(section.name) : 'Section';
        doc.text(sectionName, margin, yPosition);
        yPosition += sectionSize * options.lineHeight;
        doc.setFontSize(options.fontSize);
        doc.setFont(undefined, 'normal');

        if (section.measures && section.measures.length > 0) {
          for (const measure of section.measures) {
            // Отображаем аккорды
            if (options.showChords && measure.chordPart) {
              doc.setFont(undefined, 'bold');
              doc.text(measure.chordPart, margin, yPosition);
            }

            // Отображаем текст с выравниванием
            if (options.showLyrics && measure.lyrics) {
              doc.setFont(undefined, 'normal');
              const lyricsText = transliterate(measure.lyrics);
              doc.text(lyricsText, fixedTextOffset, yPosition);
            }

            yPosition += lineHeight;

            if (yPosition > 270) {
              doc.addPage();
              yPosition = 20;
              doc.setFontSize(options.fontSize);
            }
          }
        }

        yPosition += lineHeight / 2; // Половина межстрочного интервала между секциями
      }
    }

    // Легенда
    if (options.showLegend && yPosition < 270) {
      const legendSize = options.fontSize - 1;
      doc.setFontSize(legendSize);
      doc.setFont(undefined, 'italic');
      doc.setTextColor(100, 100, 100);

      const legend = [
        options.capo > 0 && `Capo: fret ${options.capo}`,
        `Instrument: ${options.instrument}`,
        'Generated by Music Converter'
      ].filter(Boolean);

      legend.forEach((line, index) => {
        if (yPosition + (index * legendSize * options.lineHeight) < 285) {
          doc.text(line, margin, yPosition + (index * legendSize * options.lineHeight));
        }
      });
    }

    return doc.output('blob');

  } catch (error) {
    console.error('PDF generation error:', error);
    throw new Error('Failed to generate PDF');
  }
};